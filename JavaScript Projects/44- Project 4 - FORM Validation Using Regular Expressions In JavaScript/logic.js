// ____________ Project 4- FORM Validation Using Regular Expressions In JavaScript ___________
...
form.addEventListener('submit', (e) => {
    ...
    if (forName && forEmail && forPhone && forAddress) {
        ...
        Alert.classList.remove('hide')
        Alert.innerHTML = "<strong>Danger!</strong> Your form has not been submitted successfully"
    }
...
function storingData() {
    ...
    myObj.forEach((element, index) => {
        html += `<tr> 
                        ...
                        <td id="${index}" onclick="deleteForm(this.id)">Delete ${index + 1}</td>
                     </tr>`
    });
    let msg = document.getElementById('msg')
    if (myObj.length <= 0) {
        msg.innerHTML = "<h3>Nothing to show please submit a form"
    } else {
